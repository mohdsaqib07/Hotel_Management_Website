...
const hotelRoom = {
 ...
 fields:[
 	defineField({
 		...
 		title:"Name",
 		type:"string",
 		validation: Rule=>Rule.required().max(50).error("Name Can only be of 50 Chracters"),
 	}),
 	defineField({
 		name:"slug",
 		type:"slug",
 	...
 	defineField({
 		...
 		title:"Description",
 		type:"text",
 		validation: Rule=>Rule.required().min(100).error("Must Contain at least 100 Chracters"),
 	}),
 	defineField({
 		name:"price",
 		title:"Price",
 	...
 	defineField({
 		...
 		validation: Rule=>Rule.required().error('coverImage is required')
 	}),
 	defineField({
 		name:"type",
 		title:"Room Type",
